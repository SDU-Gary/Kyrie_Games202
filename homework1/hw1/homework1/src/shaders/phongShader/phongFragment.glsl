...
#define NEAR_PLANE 0.01 // Near clipping plane
#define LIGHT_WORLD_SIZE 5. // World space size of the directional light frustum
...
varying vec4 vPositionFromLight;////vPositionFromLight为光源空间下投影的裁剪坐标
...
float PCF(sampler2D shadowMap, vec4 coords, float biasC, float filterRadiusUV){
  ...
  for(int i = 0; i < NUM_SAMPLES; i++){
    ...
    shadowDepth = useShadowMap(shadowMap, coords + offset4, biasC, filterRadiusUV);
...
void main(void) {
  ...
  //visibility = PCF(uShadowMap, vec4(shadowCoord, 1.0), PCFbias, filterRadiusUV);
  visibility = PCSS(uShadowMap, vec4(shadowCoord, 1.0), PCFbias);
